...
2.  **Run the script:**
    ...
    -   `/s sigma_value`: (Optional) A number to control the smoothness of the camera motion (e.g., `/s 10`). Defaults to 5.
    -   `/o "path\\to\\output.mp4"`: (Optional) The path where the cropped video will be saved. If not provided, it will be saved in the same directory as the input video.
